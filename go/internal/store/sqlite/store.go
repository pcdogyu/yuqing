...
CREATE INDEX IF NOT EXISTS idx_a_stock_candidate_audit_runs_lookup ON a_stock_recommendation_candidate_audit_runs(strategy_date, period, phase, created_at DESC);
...
CREATE TABLE IF NOT EXISTS a_stock_sector_fund_flow_intraday_snapshots (
	trade_date TEXT NOT NULL,
	...
	created_at TEXT NOT NULL,
	updated_at TEXT NOT NULL,
	PRIMARY KEY (trade_date, capture_time, sector_type, indicator, name)
);
...
CREATE INDEX IF NOT EXISTS idx_a_stock_sector_fund_flow_intraday_time ON a_stock_sector_fund_flow_intraday_snapshots(trade_date DESC, sector_type, indicator, capture_time);
CREATE INDEX IF NOT EXISTS idx_a_stock_sector_fund_flow_intraday_name ON a_stock_sector_fund_flow_intraday_snapshots(trade_date DESC, sector_type, indicator, name);
...
`
	...
	_, _ = s.db.ExecContext(ctx, `ALTER TABLE stock_research_surveys ADD COLUMN source_text TEXT NOT NULL DEFAULT ''`)
	_, _ = s.db.ExecContext(ctx, `ALTER TABLE stock_research_surveys ADD COLUMN source_fetch_status TEXT NOT NULL DEFAULT ''`)
	_, _ = s.db.ExecContext(ctx, `ALTER TABLE stock_research_surveys ADD COLUMN source_fetch_error TEXT NOT NULL DEFAULT ''`)
	_, _ = s.db.ExecContext(ctx, `ALTER TABLE stock_research_surveys ADD COLUMN source_fetched_at TEXT NOT NULL DEFAULT ''`)
