...
type pageData struct {
	...
	AStockAlgorithm            model.AStockRecommendationAlgorithmSettings
...
func (s *Server) Router() http.Handler {
	...
	mux.HandleFunc("/a-stock/margin", s.requireSession(s.handleAStockMarginPage))
	...
	mux.HandleFunc("/a-stock/candidates", s.requireSession(s.handleAStockCandidateAuditPage))
...
func (s *Server) handleSystem(w http.ResponseWriter, r *http.Request, user any) {
	...
	if r.Method == http.MethodPost {
		...
		case "astock_algorithm_save":
			settings, err := decodeAStockAlgorithmSettingsForm(r.PostForm)
			if err != nil {
				message = "股票推荐算法保存失败：" + err.Error()
			} else {
				resp, err := s.client.R().SetBody(settings).Put(s.cfg.ContentURL + "/api/v1/system/a-stock-recommendation-algorithm")
				if err != nil || !resp.IsSuccess() {
					message = "股票推荐算法保存失败：" + responseErrorMessage(resp, err)
				} else {
					s.clearAStockPageCaches()
					message = "股票推荐算法参数已保存，后续重新生成推荐时生效"
				}
			}
		case "astock_algorithm_reset":
			settings := defaultAStockAlgorithmSettings()
			resp, err := s.client.R().SetBody(settings).Put(s.cfg.ContentURL + "/api/v1/system/a-stock-recommendation-algorithm")
			if err != nil || !resp.IsSuccess() {
				message = "股票推荐算法恢复默认失败：" + responseErrorMessage(resp, err)
			} else {
				s.clearAStockPageCaches()
				message = "股票推荐算法参数已恢复默认"
			}
		case "database_check":
			resp, err := s.client.R().SetBody(map[string]any{
		...
		case "astock_repair_snapshot_save":
			err := s.saveAStockRepairSnapshot(aStockRepairSnapshotInput{
				...
				NewsSummaryJSON:          r.FormValue("snapshot_news_summary_json"),
		...
		case "astock_news_source_setting":
			sourceType := strings.TrimSpace(r.FormValue("source_type"))
			setting := strings.TrimSpace(r.FormValue("setting"))
			enabled := normalizeAStockBool(r.FormValue("enabled"))
			if _, ok := astocknews.SourceByType(sourceType); !ok {
				message = "新闻来源开关保存失败：未知来源"
			} else if err := astocknews.UpdateSetting("", sourceType, setting, enabled); err != nil {
				message = "新闻来源开关保存失败：" + err.Error()
			} else {
				s.clearAStockPageCaches()
				label := astocknews.Label(sourceType)
				if strings.Contains(strings.ToLower(setting), "recommend") {
					message = label + " 推荐已" + map[bool]string{true: "开启", false: "关闭"}[enabled]
				} else {
					message = label + " 抓取已" + map[bool]string{true: "开启", false: "关闭"}[enabled]
				}
			}
		}
		target := url.Values{}
		...
		if sectionKey == "newsstats" {
			target.Set("strategy_date", aStockNewsStatsDate)
		}
	...
	databaseConfig := model.DatabaseConfigStatus{}
	releaseSettings := model.ReleaseSettings{}
	aStockAlgorithm := defaultAStockAlgorithmSettings()
	warningSetting := model.WarningSetting{}
	projects := []model.Project{}
	groups := []model.ProjectGroup{}
	...
	if sectionKey == "stockalgo" {
		aStockAlgorithm = s.loadAStockAlgorithmSettings()
	}
...
func normalizeSystemSection(section string) string {
	...
	case "preferences", "preference":
		return "preferences"
	case "stockalgo", "stock_algorithm", "stock-algorithm", "astockalgo", "a-stock-algorithm":
		return "stockalgo"
...
const portalNavHTML = `<nav><a href="/">总览</a><a href="/articles">文章</a><a href="/hotspots">热点</a><a href="/a-stock">A股</a><a href="/a-stock/backtest">回测</a><a href="/sector-fund-flow">版块资金</a><a href="/stock-research">研报调研</a><a href="/a-stock/margin">融资融券</a><a href="/a-stock/holdings">机构持仓</a><a href="/a-stock/auction">集合竞价</a><a href="/crypto">Crypto</a><a href="/system">系统</a><a href="/logs">日志</a><button id="portal-upgrade-button" class="portal-upgrade-button" type="button">升级</button><a class="logout-link" href="/logout">退出</a></nav>`
...
var consoleState=byId("portal-upgrade-console-state");
...
function shouldStickToBottom(){return !log||log.scrollTop+log.clientHeight>=log.scrollHeight-32}
...
.fundflow-chart{width:100%;height:865px;display:block}
...
.fundflow-rank-list{max-height:865px;overflow:auto}
...
<div class="fundflow-chart-wrap"><svg class="fundflow-chart" id="hotspot-fundflow-chart" viewBox="0 0 1104 865" role="img" aria-label="当日板块资金流向折线图"></svg></div>
...
function parseTradeMinute(value){var m=String(value||"").match(/(\d{1,2}):(\d{2})/);if(!m){return null}return Number(m[1])*60+Number(m[2])}
function smoothSeriesPoints(points){var rows=(points||[]).map(function(p,idx){return {time:p.time,minute:parseTradeMinute(p.time),value:Number(p.main_net_inflow||0),rank:p.rank,idx:idx}});return rows.map(function(row,idx){var values=[];for(var j=Math.max(0,idx-4);j<=idx;j++){values.push(rows[j].value)}var avg=values.reduce(function(sum,value){return sum+value},0)/values.length;return {time:row.time,minute:row.minute,main_net_inflow:avg,rank:row.rank}})}
function bestFundFlowPoint(points){var best=null;(points||[]).forEach(function(p){var value=Number(p.main_net_inflow||0);if(!best||Math.abs(value)>Math.abs(Number(best.main_net_inflow||0))){best=p}});return best}
function latestFundFlowPoint(points){var list=(points||[]).filter(function(p){return parseTradeMinute(p.time)!==null});return list.length?list[list.length-1]:null}
function renderChart(data){
 ...
 var height=865,width=Math.max(1214,Math.round((svg.getBoundingClientRect&&svg.getBoundingClientRect().width)||svg.clientWidth||1214));svg.setAttribute("viewBox","0 0 "+width+" "+height);
 if(!series.length||!times.length){svg.appendChild(svgEl("text",{x:String(width/2),y:"433","text-anchor":"middle",fill:"#6a6257"})).textContent="暂无日内资金快照";return}
 var left=58,right=238,top=38,bottom=92,plotW=width-left-right,plotH=height-top-bottom,zeroY=top+plotH/2;
 ...
 var maxOutflow=values.reduce(function(max,value){return Math.max(max,Math.abs(Math.min(value,0)))},0);
 var fallbackAbsMax=values.reduce(function(max,value){return Math.max(max,Math.abs(value))},0);
 var yStep=10,yMaxInflow=Math.ceil(Math.max((maxInflow>0?maxInflow:fallbackAbsMax)*1.10,yStep)/yStep)*yStep,yMaxOutflow=Math.ceil(Math.max((maxOutflow>0?maxOutflow:fallbackAbsMax)*1.10,yStep)/yStep)*yStep;
 if(!isFinite(yMaxInflow)||yMaxInflow<=0){yMaxInflow=yStep} if(!isFinite(yMaxOutflow)||yMaxOutflow<=0){yMaxOutflow=yStep}
 function yForValue(value){var yi=rawYi(value),scale=yi>=0?yMaxInflow:yMaxOutflow;var y=yi>=0?zeroY-yi/scale*plotH/2:zeroY+Math.abs(yi)/scale*plotH/2;return Math.max(top,Math.min(height-bottom,y))}
 function labelEveryFor(max){return max>120?50:(max>60?20:10)}
 function drawYGrid(tick,scale){var y=tick>=0?zeroY-tick/scale*plotH/2:zeroY+Math.abs(tick)/scale*plotH/2;var isZero=Math.abs(tick)<0.0001;svg.appendChild(svgEl("line",{x1:left,y1:y,x2:width-right,y2:y,stroke:isZero?"#9a8f7d":"#d8cfbf","stroke-width":isZero?"1.6":"1","stroke-opacity":isZero?"1":"0.62"}));var labelEvery=labelEveryFor(scale);if(isZero||Math.abs(tick)===scale||Math.abs(tick)%labelEvery===0){var label=svgEl("text",{x:left-10,y:y+4,"text-anchor":"end",fill:"#6a6257","font-size":"13"});label.textContent=tick.toFixed(0)+"亿";svg.appendChild(label)}}
 for(var upTick=0;upTick<=yMaxInflow+0.0001;upTick+=yStep){drawYGrid(upTick,yMaxInflow)}
 for(var downTick=-yStep;downTick>=-yMaxOutflow-0.0001;downTick-=yStep){drawYGrid(downTick,yMaxOutflow)}
 ...
 var endLabels=[];
 displaySeries.forEach(function(s,idx){var path=[];(s.points||[]).forEach(function(p){var x=xForTime(p.time);if(x===null){return}var y=yForValue(p.main_net_inflow);path.push((path.length?"L":"M")+x.toFixed(1)+" "+y.toFixed(1))});if(!path.length){return}var color=colors[idx%colors.length];svg.appendChild(svgEl("path",{d:path.join(" "),fill:"none",stroke:color,"stroke-width":"2.4","stroke-linejoin":"round","stroke-linecap":"round"}));var latestPoint=latestFundFlowPoint(s.points);if(latestPoint&&rawYi(latestPoint.main_net_inflow)!==0){var x=xForTime(latestPoint.time);if(x===null){return}var y=yForValue(latestPoint.main_net_inflow);svg.appendChild(svgEl("circle",{cx:x,cy:y,r:"3.5",fill:color}));endLabels.push({x:x,y:y,labelX:Math.min(Math.max(x+10,left+8),width-216),color:color,text:s.name+" "+moneyYi(latestPoint.main_net_inflow)})}});
 ...
 endLabels.forEach(function(item){svg.appendChild(svgEl("path",{d:"M"+item.x.toFixed(1)+" "+item.y.toFixed(1)+" L"+(item.labelX-4).toFixed(1)+" "+item.labelY.toFixed(1),fill:"none",stroke:item.color,"stroke-width":"1","stroke-opacity":"0.45"}));var label=svgEl("text",{x:item.labelX,y:item.labelY+4,fill:item.color,"font-size":"12","font-weight":"700"});label.textContent=item.text;svg.appendChild(label)})
}
...
func buildSystemTemplate() string {
	...
	template = strings.Replace(template,
		...
		`.newsstats-filter{margin-bottom:14px}.stockalgo-form{display:block}.stockalgo-group{margin-top:18px}.stockalgo-group h3{margin:0 0 10px}.stockalgo-table{min-width:980px;table-layout:fixed}.stockalgo-table th,.stockalgo-table td{vertical-align:top}.stockalgo-table input{width:100%;box-sizing:border-box}.stockalgo-actions{display:flex;gap:10px;margin-top:18px;align-items:center}.astock-news-grid{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:16px;align-items:start}.astock-news-window{min-width:0}.astock-news-table{table-layout:fixed}.astock-news-table th,.astock-news-table td{vertical-align:top}.astock-source-link{color:#214e34;font-weight:700;text-decoration:none}.astock-source-toggle-form{width:auto;margin:0}.astock-source-toggle{display:inline-flex;align-items:center;justify-content:center;width:42px;min-width:42px;height:30px;margin:0;padding:0;border-radius:8px;font-size:13px;font-weight:700}.astock-source-toggle.on{background:#214e34;color:#fff}.astock-source-toggle.off{background:#8f2d2d;color:#fff}.astock-news-counts{display:inline-flex;align-items:center;gap:6px;white-space:nowrap}.astock-news-diagnostic{margin-top:6px;color:#8a5a17;font-size:12px;line-height:1.35}.astock-empty{padding:18px;border:1px dashed #d0c8b8;border-radius:12px;background:#fff;color:#6a6257}.astock-scroll{width:100%;overflow:auto}.astock-help{display:inline-flex;align-items:center;justify-content:center;width:18px;height:18px;border-radius:50%;background:#eef4ec;color:#214e34;font-size:12px;font-weight:700;line-height:1;cursor:help;position:relative}.astock-help-text{position:absolute;right:0;top:calc(100% + 8px);z-index:10;display:none;width:max-content;max-width:260px;padding:8px 10px;border:1px solid #d6ccbb;border-radius:8px;background:#fff;color:#2b261f;box-shadow:0 12px 28px rgba(31,40,34,.14);font-size:12px;font-weight:400;line-height:1.4;white-space:normal}.astock-help:hover .astock-help-text,.astock-help:focus .astock-help-text{display:block}@media (max-width:1100px){.astock-news-grid{grid-template-columns:1fr}.astock-news-table{min-width:900px}}</style></head><body><header><h1>系统工作台</h1>`,
		1,
	)
	template = strings.Replace(template,
		`.stockalgo-table{min-width:980px;table-layout:fixed}.stockalgo-table th,.stockalgo-table td{vertical-align:top}.stockalgo-table input{width:100%;box-sizing:border-box}`,
		`.stockalgo-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.stockalgo-param{display:grid;grid-template-columns:minmax(150px,1.05fr) minmax(120px,.8fr) minmax(36px,max-content) minmax(180px,1.25fr);gap:8px;align-items:center;padding:10px;border:1px solid #ece7dc;border-radius:8px;background:#fff}.stockalgo-param-name,.stockalgo-param-current,.stockalgo-param-unit,.stockalgo-param-desc{display:flex;align-items:center;min-height:42px}.stockalgo-param-name{display:block}.stockalgo-param-current input{width:100%;box-sizing:border-box;margin:0}.stockalgo-param-unit{justify-content:flex-start;white-space:nowrap}.stockalgo-param-unit,.stockalgo-param-desc{color:#2b261f}@media (max-width:1200px){.stockalgo-grid{grid-template-columns:1fr}.stockalgo-param{grid-template-columns:minmax(140px,1fr) minmax(120px,.8fr) minmax(36px,max-content) minmax(160px,1fr)}}`,
		1,
	)
	template = strings.Replace(template,
		`{{range .AStockAlgorithmGroups}}<div class="stockalgo-group"><h3>{{.Label}}</h3><div class="astock-scroll"><table class="stockalgo-table"><thead><tr><th>参数</th><th>当前值</th><th>默认值</th><th>单位</th><th>说明</th></tr></thead><tbody>{{range .Params}}<tr><td><strong>{{.Label}}</strong><div class="muted">{{.Key}}</div></td><td><input type="{{.InputType}}" step="{{.Step}}" name="param.{{.Key}}" value="{{.Value}}"></td><td>{{.DefaultValue}}</td><td>{{.Unit}}</td><td>{{.Description}}</td></tr>{{end}}</tbody></table></div></div>{{end}}`,
		`{{range .AStockAlgorithmGroups}}<div class="stockalgo-group"><h3>{{.Label}}</h3><div class="stockalgo-grid">{{range .Params}}<div class="stockalgo-param"><div class="stockalgo-param-name"><strong>{{.Label}}</strong><div class="muted">{{.Key}}</div></div><div class="stockalgo-param-current"><input type="{{.InputType}}" step="{{.Step}}" name="param.{{.Key}}" value="{{.Value}}"></div><div class="stockalgo-param-unit">{{.Unit}}</div><div class="stockalgo-param-desc">{{.Description}}</div></div>{{end}}</div></div>{{end}}`,
		1,
	)
