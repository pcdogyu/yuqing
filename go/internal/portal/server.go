...
func (s *Server) handleArticleDetail(w http.ResponseWriter, r *http.Request, user any) {
	...
	if r.Method == http.MethodPost && userID > 0 {
		...
		if action == "delete" && message == "文章已删除" {
...
const portalNavHTML = `<nav><a href="/">总览</a><a href="/projects">项目</a><a href="/monitor-rules">规则</a><a href="/articles">文章</a><a href="/reports">报告</a><a href="/crawl-templates">模板中心</a><a href="/crawl-templates/manage">模板管理</a><a href="/crypto">Crypto</a><a href="/system">系统</a><a class="logout-link" href="/logout">退出</a></nav>`
...
const portalSourceOptions = `<option value="">全部来源</option><option value="flash">flash</option><option value="headline">headline</option><option value="crypto_x">crypto_x</option><option value="crypto_telegram">crypto_telegram</option><option value="foresight_newsflash">foresight_newsflash</option><option value="coindesk_zh_latest">coindesk_zh_latest</option><option value="panews_newsflash">panews_newsflash</option>`
const portalSourceFilterOptions = `<option value="">全部来源</option><option value="flash" {{if eq .FilterSource "flash"}}selected{{end}}>flash</option><option value="headline" {{if eq .FilterSource "headline"}}selected{{end}}>headline</option><option value="crypto_x" {{if eq .FilterSource "crypto_x"}}selected{{end}}>crypto_x</option><option value="crypto_telegram" {{if eq .FilterSource "crypto_telegram"}}selected{{end}}>crypto_telegram</option><option value="foresight_newsflash" {{if eq .FilterSource "foresight_newsflash"}}selected{{end}}>foresight_newsflash</option><option value="coindesk_zh_latest" {{if eq .FilterSource "coindesk_zh_latest"}}selected{{end}}>coindesk_zh_latest</option><option value="panews_newsflash" {{if eq .FilterSource "panews_newsflash"}}selected{{end}}>panews_newsflash</option>`
const portalChannelPlaceholder = `来源，如 flash,headline,crypto_x,crypto_telegram,foresight_newsflash,coindesk_zh_latest,panews_newsflash`
