...
function renderChart(data){
 ...
 [["09:30",tradeStart],["10:30",10*60+30],["11:30",11*60+30],["13:00",13*60],["14:00",14*60],["15:00",tradeEnd]].forEach(function(item){var x=left+(item[1]-tradeStart)/(tradeEnd-tradeStart)*plotW;svg.appendChild(svgEl("line",{x1:x,y1:top,x2:x,y2:height-bottom,stroke:"#ece7dc","stroke-width":"1"}));var label=svgEl("text",{x:x,y:height-18,"text-anchor":"middle",fill:"#6a6257","font-size":"13"});label.textContent=item[0];svg.appendChild(label)});
...
func buildSystemTemplate() string {
	...
	template = strings.Replace(template,
		...
		`.stockalgo-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.stockalgo-param{display:grid;grid-template-columns:minmax(150px,1.05fr) minmax(120px,.8fr) minmax(96px,.6fr) minmax(64px,.4fr) minmax(180px,1.15fr);gap:10px;align-items:center;padding:10px;border:1px solid #ece7dc;border-radius:8px;background:#fff}.stockalgo-param-name,.stockalgo-param-current,.stockalgo-param-default,.stockalgo-param-unit,.stockalgo-param-desc{display:flex;align-items:center;min-height:42px}.stockalgo-param-name{display:block}.stockalgo-param-current input{width:100%;box-sizing:border-box;margin:0}.stockalgo-param-default,.stockalgo-param-unit,.stockalgo-param-desc{color:#2b261f}@media (max-width:1200px){.stockalgo-grid{grid-template-columns:1fr}.stockalgo-param{grid-template-columns:minmax(140px,1fr) minmax(120px,.8fr) minmax(90px,.6fr) minmax(60px,.35fr) minmax(160px,1fr)}}`,
		1,
	)
	template = strings.Replace(template,
		...
		`{{range .AStockAlgorithmGroups}}<div class="stockalgo-group"><h3>{{.Label}}</h3><div class="stockalgo-grid">{{range .Params}}<div class="stockalgo-param"><div class="stockalgo-param-name"><strong>{{.Label}}</strong><div class="muted">{{.Key}}</div></div><div class="stockalgo-param-current"><input type="{{.InputType}}" step="{{.Step}}" name="param.{{.Key}}" value="{{.Value}}"></div><div class="stockalgo-param-default">{{.DefaultValue}}</div><div class="stockalgo-param-unit">{{.Unit}}</div><div class="stockalgo-param-desc">{{.Description}}</div></div>{{end}}</div></div>{{end}}`,
		1,
	)
