...
func (s *Store) ListCrawlTemplates(ctx context.Context) ([]model.CrawlTemplate, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT id, name, source_type, enabled, config_json, created_at, updated_at FROM crawl_templates ORDER BY updated_at DESC, id DESC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) GetCrawlTemplate(ctx context.Context, id int64) (model.CrawlTemplate, error) {
	row := s.db.QueryRowContext(ctx, `SELECT id, name, source_type, enabled, config_json, created_at, updated_at FROM crawl_templates WHERE id = ?`, id)
...
func (s *Store) CreateCrawlTemplate(ctx context.Context, tpl model.CrawlTemplate) (model.CrawlTemplate, error) {
	...
	tpl.Enabled = true
	res, err := s.db.ExecContext(ctx, `INSERT INTO crawl_templates (name, source_type, enabled, config_json, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)`,
		tpl.Name, tpl.SourceType, boolToInt(tpl.Enabled), tpl.ConfigJSON, now.Format(time.RFC3339), now.Format(time.RFC3339))
	if err != nil {
		return model.CrawlTemplate{}, err
	}
...
func (s *Store) UpdateCrawlTemplate(ctx context.Context, tpl model.CrawlTemplate) (model.CrawlTemplate, error) {
	...
	_, err := s.db.ExecContext(ctx, `UPDATE crawl_templates SET name = ?, source_type = ?, enabled = ?, config_json = ?, updated_at = ? WHERE id = ?`,
		tpl.Name, tpl.SourceType, boolToInt(tpl.Enabled), tpl.ConfigJSON, tpl.UpdatedAt.Format(time.RFC3339), tpl.ID)
	if err != nil {
		return model.CrawlTemplate{}, err
	}
	return s.GetCrawlTemplate(ctx, tpl.ID)
...
func scanCrawlTemplate(scanner scanner) (model.CrawlTemplate, error) {
	...
	if err := scanner.Scan(&tpl.ID, &tpl.Name, &tpl.SourceType, &enabled, &tpl.ConfigJSON, &createdAt, &updatedAt); err != nil {
		return model.CrawlTemplate{}, err
	}
