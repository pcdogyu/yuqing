...
func (s *Server) handleProjects(w http.ResponseWriter, r *http.Request, user any) {
	...
	if keyword != "" {
		...
		for _, group := range groups {
			if strings.Contains(strings.ToLower(group.Name), lowerKeyword) || strings.Contains(strings.ToLower(group.Description), lowerKeyword) {
...
func (s *Server) handleProjectDetail(w http.ResponseWriter, r *http.Request, user any) {
	...
	_ = s.render(w, "project", pageData{Title: "项目详情", User: user, Project: project, Groups: groups, Rules: rules, Articles: articles, CrawlRuns: crawlRuns, Reports: reports, TaskRuns: taskRuns, Message: r.URL.Query().Get("msg")})
}
...
func (s *Server) handleReportDetail(w http.ResponseWriter, r *http.Request, user any) {
	...
	_ = s.render(w, "report", pageData{Title: "报告详情", User: user, Report: report, Project: project, Articles: articles, CrawlRuns: crawlRuns, TaskRuns: taskRuns, Message: r.URL.Query().Get("msg"), ReturnURL: returnURL, ReturnTo: url.QueryEscape(returnURL)})
}
