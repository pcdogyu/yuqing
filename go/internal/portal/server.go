...
function smoothSeriesPoints(points){var rows=(points||[]).map(function(p,idx){return {time:p.time,minute:parseTradeMinute(p.time),value:Number(p.main_net_inflow||0),rank:p.rank,idx:idx}});return rows.map(function(row,idx){var values=[];rows.forEach(function(other,j){if(row.minute!==null&&other.minute!==null){if(other.minute>=row.minute-2&&other.minute<=row.minute){values.push(other.value)}}else if(j>=idx-2&&j<=idx){values.push(other.value)}});if(!values.length){values=[row.value]}var avg=values.reduce(function(sum,value){return sum+value},0)/values.length;return {time:row.time,minute:row.minute,main_net_inflow:avg,rank:row.rank}})}
