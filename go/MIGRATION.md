...
- 二期范围已确认：以 `MIGRATION.md` 的“部分承接 / 已承接/兼容”中仍依赖旧 Java 的能力为主，并覆盖 `README.md` 中标记的未迁移项。
- 切换策略：先并行验证，再切流到 Go；二期完成后不再把旧兼容入口当作长期方案。
- 当前进度：
  - 已梳理 Java 控制器与 Go 侧现状映射。
  - 已确认 Go 侧现有服务边界与路由结构。
  - 已开始收口 `portal-web` 中的 legacy 兼容路由。
- 下一步：
  - 补齐二期接口与数据闭环。
  - 清理对应 legacy 兼容路由和旧跳转入口。
  - 补充单测与端到端验收。
