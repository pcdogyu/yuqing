...
A 股策略工作台 `/a-stock` 默认按 `Asia/Shanghai` 增加三个推荐任务：上午 `a-stock-morning-recommendation-preview` 每日 `09:26` 延迟 2 分钟准备快照、`a-stock-morning-recommendation` 每日 `09:32` 再生成；下午 `a-stock-afternoon-recommendation` 每日 `13:00` 生成。上午推荐按 `08:00-09:24` 与 `08:00-09:30` 两个快照合并去重；下午推荐只按 `09:30-13:00` 单窗口生成，并过滤当天上午已推荐股票。任务会触发 `flash`、`headline`、`jin10_full`、`eastmoney_kuaixun`、`wallstreetcn_a_stock`、`cls_telegraph`、`sina_finance_7x24` 新闻源，可用对应 `YUQING_SCHEDULER_<JOB_NAME>_CRON` 覆盖执行时间。
